...
public:
  ...
  void parseOptions(int argc, char* argv[]) {
    int c;
    while ((c = getopt(argc, argv, ":h?adtmC:F:H:S:W:E:G:B:D:")) != -1) {
  ...
  bool isVideo() {
    ...
    void processMagThetaImages(cv::Mat& start, cv::Mat& mag, cv::Mat& theta)
    {
        double t0;
        if (m_timing) {
            t0 = tic();
        ...
        if (m_draw) {
            for (int i=0; i<detections.size(); i++) {
                // also highlight in the image
                detections[i].draw(start);
            }
            imshow(windowName, start); // OpenCV call
        }
    ...
    void loadMagThetaImages()
    {
        cv::Mat start;
        cv::Mat mag;
        cv::Mat theta;

        list<string>::iterator it=m_imgNames.begin();

        start = cv::imread(*it, cv::IMREAD_GRAYSCALE);
        start.convertTo(start, CV_32FC1, (1. / 255.));
        mag = cv::imread(*(++it), cv::IMREAD_GRAYSCALE);
        mag.convertTo(mag, CV_32FC1, (1. / 255.));
        theta = cv::imread(*(++it), cv::IMREAD_GRAYSCALE);
        theta.convertTo(theta, CV_32FC1, ((2. * 3.14159265358979323846) / 255.), -3.14159265358979323846);

        processMagThetaImages(start, mag, theta);
