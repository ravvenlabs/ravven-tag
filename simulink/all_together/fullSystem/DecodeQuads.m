function TagDetections = DecodeQuads(quads,GrayImg,Debug)
    %Constants to export
    ...
    %         ThisQuad = optimize_quad_generic(tag36h11,GrayImg,ThisQuad,refineSteps);
            ...
            %Calculate the perimeter of the tag
            dist1 = Pt2PtDist(ThisQuad.p(1,1),ThisQuad.p(1,2),ThisQuad.p(2,1),ThisQuad.p(2,2));
            dist2 = Pt2PtDist(ThisQuad.p(2,1),ThisQuad.p(2,2),ThisQuad.p(3,1),ThisQuad.p(3,2));
            dist3 = Pt2PtDist(ThisQuad.p(3,1),ThisQuad.p(3,2),ThisQuad.p(4,1),ThisQuad.p(4,2));
            dist4 = Pt2PtDist(ThisQuad.p(4,1),ThisQuad.p(4,2),ThisQuad.p(1,1),ThisQuad.p(1,2));
            TagDetection.obsPerimeter = dist1 + dist2 + dist3 + dist4;
...
function GrayModel = GM_compute(GrayModel)
    ...
    if(GrayModel.nobs >= 6)
        ...
        GrayModel.v(4,1) = GrayModel.b(3,1) / GrayModel.nobs;
    end
end
...
function TagDetection = TF_Decode(rCode)
    ...
    for id = 1:size(TagFamily.Codes,2)
        for rot = 1:5
            thisHamming = HammingDistMex(rCodes(rot),TagFamily.Codes(id));
