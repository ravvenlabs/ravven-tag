...
namespace AprilTag {
    ...
    template<class T>
    void cvToFloatImage(const cv::Mat& mat, FloatImage& toRet, double scale = 1.0)
    {
...
#pragma omp parallel for
        ...
        { // limit scope of storage
            ...
            for (int y = 0; y + 1 < height; y++) {
                for (int x = 0; x + 1 < width; x++) {
                    ...
                    Edge::calcEdges(theta0, x, y, step4.fimTheta, step4.fimMag, edges, nEdges);
    ...
    void showStep7(Step_7 step7, Step_1 step1)
    {
        ...
        for (std::vector<Segment>::iterator segment = step7.segments.begin(); segment != step7.segments.end(); segment++)
        {
    ...
    Step_8 connectSegments(Step_7* step7, int width, int height)
    {
        ...
        for (unsigned i = 0; i < step7->segments.size(); i++) {
            Segment& parentseg = step7->segments[i];
    ...
    void showStep8(Step_7 step7, Step_1 step1)
    {
        ...
        while (segmentIdList.size() > 0)
        {
            ...
            Segment segment = step7.segments[currSegId];
            for (std::vector<Segment*>::iterator segmentChild = segment.children.begin(); segmentChild != segment.children.end(); segmentChild++)
            {
    ...
    void showStep9(Step_9 step9, Step_1 step1)
    {
        cv::Mat baseImage(step1.fimOrig.rows, step1.fimOrig.cols, CV_8UC3);
        cv::cvtColor(step1.fimOrig, baseImage, cv::COLOR_GRAY2BGR, 3);
        for (std::vector<Quad>::iterator quad = step9.quads.begin(); quad != step9.quads.end(); quad++)
        {
            cv::Scalar color((rand() % 255) / 255., (rand() % 255) / 255., (rand() % 255) / 255.);
            cv::Rect2d rect(
     ...
     */
    Step_10 decodeQuads(Step_2 step2, Step_9 step9, int width, int height, TagFamily tagFamily)
    {
        std::vector<TagDetection> detections;

        for (unsigned int qi = 0; qi < step9.quads.size(); qi++) {
            Quad& quad = step9.quads[qi];
    ...
    void showStep10(Step_10 step10, Step_1 step1)
    {
        cv::Mat baseImage(step1.fimOrig.rows, step1.fimOrig.cols, CV_8UC3);
        cv::cvtColor(step1.fimOrig, baseImage, cv::COLOR_GRAY2BGR, 3);
        for (std::vector<TagDetection>::iterator detect = step10.detections.begin(); detect != step10.detections.end(); detect++)
        {
    ...
    void showStep11(Step_11 step11, Step_1 step1)
    {
        cv::Mat baseImage(step1.fimOrig.rows, step1.fimOrig.cols, CV_8UC3);
        cv::cvtColor(step1.fimOrig, baseImage, cv::COLOR_GRAY2BGR, 3);
        for (std::vector<TagDetection>::iterator detect = step11.goodDetections.begin(); detect != step11.goodDetections.end(); detect++)
        {
            detect->draw(baseImage);
        }
