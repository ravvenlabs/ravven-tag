-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj1\hdlsrc\gaussianFilter\gaussianF_ip_src_y_FIFO.vhd
...
-- -------------------------------------------------------------
-- 
-- Module: gaussianF_ip_src_y_FIFO
-- Source Path: gaussianFilter/gaussianFilter/Output_FIFOs/y_FIFO
-- Hierarchy Level: 2
-- Model version: 8.37
-- 
-- -------------------------------------------------------------
...
ENTITY gaussianF_ip_src_y_FIFO IS
...
END gaussianF_ip_src_y_FIFO;


ARCHITECTURE rtl OF gaussianF_ip_src_y_FIFO IS
  ...
  SIGNAL y_out_tmp                        : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL has_space_1                      : std_logic;
...
BEGIN
  ...
  push_1 <= valid_in AND enable;

  fwft_wr_en <= cache_wr_en OR out_wr_en;

  R_x <=  NOT fwft_wr_en;

  Q_keep <= R_x AND fifo_valid;

  fifo_and_out_valid <= fifo_valid AND out_valid;

  R_x_1 <=  NOT out_wr_en;

  Q_keep_1 <= R_x_1 AND cache_valid;
  ...
  valid_out_1 <=  NOT buffer_empty;

  pop <= ready_in AND valid_out_1;

  data_flow <= pop OR buffer_empty;
  ...
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        y_out_tmp <= to_unsigned(0, 32);
      ELSIF enb = '1' AND out_wr_en = '1' THEN
        y_out_tmp <= data_out_next;
      END IF;
    END IF;
  ...
  has_space_1 <=  NOT buffer_full;

  y_has_space <= has_space_1;
