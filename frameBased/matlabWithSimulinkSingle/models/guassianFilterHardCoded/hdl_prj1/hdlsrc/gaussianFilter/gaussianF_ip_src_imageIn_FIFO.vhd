-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj1\hdlsrc\gaussianFilter\gaussianF_ip_src_imageIn_FIFO.vhd
...
-- -------------------------------------------------------------
-- 
-- Module: gaussianF_ip_src_imageIn_FIFO
-- Source Path: gaussianFilter/gaussianFilter/Input_FIFOs/imageIn_FIFO
-- Hierarchy Level: 2
-- Model version: 8.39
-- 
-- -------------------------------------------------------------
...
ENTITY gaussianF_ip_src_imageIn_FIFO IS
...
END gaussianF_ip_src_imageIn_FIFO;


ARCHITECTURE rtl OF gaussianF_ip_src_imageIn_FIFO IS
  ...
  SIGNAL imageIn_out_tmp                  : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL hasData_1                        : std_logic;

BEGIN
  -- us1: Upsample by 1, Sample offset 0 
  u_FIFO_classic_ram_generic : gaussianF_ip_src_SimpleDualPortRAM_generic
  ...
  fifo_nfull <=  NOT buffer_full;

  ready_out_1 <=  NOT buffer_full;

  push_1 <= valid_in AND ready_out_1;

  -- us2: Upsample by 1, Sample offset 0 
  fifo_push <= push_1 AND fifo_nfull;
  ...
  buffer_empty <=  NOT out_valid;

  data_flow <= should_read OR buffer_empty;
  ...
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        imageIn_out_tmp <= to_unsigned(0, 32);
      ELSIF enb = '1' AND out_wr_en = '1' THEN
        imageIn_out_tmp <= data_out_next;
      END IF;
    END IF;
  END PROCESS out_data_reg_process;


  imageIn_out <= std_logic_vector(imageIn_out_tmp);

  imageIn_ready <= ready_out_1;

  hasData_1 <=  NOT buffer_empty;

  imageIn_has_data <= hasData_1;
