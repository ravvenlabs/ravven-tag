...
imageStack = single(ones(height,width,2));
imageStack(:,:,1) = image_gray;
imageStack(:,:,2) = image_gray;
...
dataLeft = read(client,width*height,"single");   
temp = reshape(dataLeft,[width,height]);
leftProcessed = permute(temp,[2 1]);

dataRight = read(client,width*height,"single");
